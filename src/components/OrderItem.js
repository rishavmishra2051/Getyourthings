...
const OrderItem = ({ id, item, status, date }) => {
    ...
    const cancelOrder = async (action) => {
        try {
            await updateDoc(doc(colRef, id), { status: action });
    ...
    const handleStatusChange = async () => {
        try {
            ...
            await updateDoc(doc(colRef, id), { status: orderaction });
    ...
    <div className='mb-4 border'>
        <div className='flex justify-between mx-2 mt-2'>
            ...
            <p className='inline-flex font-titleFont font-semibold'>Status: <p className='font-titleFont font-semibold' style={{ color: status === 'Delivered' ? 'green' : status === 'Cancelled' ? 'red' : status === 'Returned' ? 'Yellow' : '#FFA500' }}>{status}</p></p>
        </div>
